...
const InternalsView = ({ appointmentId }) => {
  ...
  useEffect(() => {
    if (!internals || !conditions || conditions.length === 0) return;
  ...
  if (loadingInternals || loadingConditions) {
    return <p className="text-gray-500">Loading internals...</p>;
  }

  if (conditionsError || !internals?.length) {
    return <p className="text-red-500">No internal observations found.</p>;
  }

  return (
    <div className="bg-white p-6 rounded shadow-md max-h-[80vh] overflow-y-auto space-y-6">
      <h2 className="text-xl font-bold text-[#2f3192]">Internal Observations</h2>
