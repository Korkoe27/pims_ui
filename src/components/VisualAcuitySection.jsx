...
export default function VisualAcuitySection({
  ...
  const getPlaceholder = () => (vaChart === "SNELLEN" ? "6/6" : "0.00");
    ...
    <div>
      ...
      {fields.length > 1 && (
        <div className="grid grid-cols-[80px_repeat(3,_1fr)] gap-4 text-sm font-medium mb-2">
          <div></div>
          {fields.map((f) => (
            <div key={f}>{f === "plusOne" ? "+1.00" : f.charAt(0).toUpperCase() + f.slice(1)}</div>
          ))}
        </div>
      )}

      <div className={`grid ${fields.length > 1 ? "grid-cols-[80px_repeat(3,_1fr)]" : "grid-cols-2"} gap-4`}>
        {EYES.map((eye) => (
