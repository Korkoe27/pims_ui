    ...
    <div className="px-8 ml-72 flex flex-col mt-8 gap-8 bg-[#f9fafb] w-full shadow-md sm:rounded-lg">
      ...
      {appointments && appointments?.data?.today_appointments && (
        <table className="w-full text-base text-left rtl:text-right text-gray-500 ">
          ...
          <tbody className="">
            {appointments?.data?.today_appointments?.data?.map(
              (appointment) => (
            ...
            {appointment?.first_name}
                  ...
                  <td className="w-fit mx-auto">
                    <span
                      className={`px-6 rounded-full py-3 text-base font-medium text-white w-5 ${checkStatus(
                        appointment?.status
                      )}`}
                    >
                      {appointment?.status}
                  ...
                  <td className="px-6 py-4 flex gap-10">
                    <button className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none dark:focus:ring-blue-800">
                      View
                    </button>
                    <button
