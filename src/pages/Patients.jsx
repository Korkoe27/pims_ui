...
const Patients = () => {
  ...
  const handleViewPatient = (patientId) => {
    navigate('/patient-details/'); // Navigate to PatientDetails with patientId
  };
            ...
            {patients?.data?.results.map((patient) => (
              <tr key={patient.id} className="bg-white border-b ">
                ...
                <td className="px-6 py-4 flex gap-10">
                  <button
                    onClick={() => handleViewPatient()} // Pass patient ID
                    className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none dark:focus:ring-blue-800"
                  >
                    View
                  </button>
                </td>
